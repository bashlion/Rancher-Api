...
##--Will only be used while deleting an empty cluster while accessing the node fails---####
RANCHER_USERNAME="RANCHER-username-here"  ##---->MODIFY
RANCHER_FQDN="rancher.domain.com"  ##----MODIFY
DELETE_COMMAND="kubectl delete clusters -n fleet-default '$CLUSTER_NAME'" ##--> DO NOT MODIFY
#############
		...
		if [ -n "$CLUSTERID" ]; then
			...
			for ip_address in "${ip_addresses[@]}"; do
				...
				if [ $? -eq 0 ]; then
					...
					ssh -v -o StrictHostKeyChecking=no "$RANCHER_USERNAME"@"$RANCHER_FQDN" "$DELETE_COMMAND"
					exit 1
				fi
			done
		else
